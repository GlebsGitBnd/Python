...
class RBTree:
	...
	def search(self, data):
		node, presence = self.find(data, None, self.root)
		if presence:
			return node.data
		else:
			return 'Значение {0} отсутствует'.format(data)
	...
	def append(self, data):
		...
		# Проверяем есть ли элемент в дереве, если нет возвращаем родителя
		dad, presence = self.find(node.data, None, self.root)
	...
	def to_fixed(self, node, dad):
		...
		# Балансируем если 2 вертки подряд красные
		if node.color == 'RED' and dad.color == 'RED':
			...
			# Если дядя черный делаем повороты, если красный, то перекрашиваем
			if uncle.color == 'BLACK':
				node, dad = self.to_low_rotate(node, dad, grand_dad)
				self.to_big_rotate(dad, grand_dad)
	...
	# Малый поворот (если срабатывает, родитель с сыном меняются местами)
	...
	# Большой поворот
	def to_big_rotate(self, dad, grand_dad):
	...
	# Удаление (если значения нет - заканчиваем, если есть - удаляем) рекурсивная
	def delete_node(self, num):
		...
		# Ситуация если 2 ребенка (ищем крайний левый элемент правого поддерева)
		if left_son.data and right_son.data:
			last_elem = self.del_node_with_two(right_son)
			
			# Возвращаем элемент с одним ребенком или без детей для swap значений
			elem = self.delete_node(last_elem.data)
			node.data, elem.data = elem.data, node.data
		
		# Ситуация если 1 ребенок
		if (left_son.data and not right_son.data) \
			or (right_son.data and not left_son.data):
		...
		# Ситуация если нет детей
		if not right_son.data and not right_son.data:
			self.del_node_without_child(node, dad)
			self.tree_show(self.root)
			return node
	
	# Нахождение минимального числа для swap (При удалении когда 2 сына)
	...
	# Удаление если ветка черная и нет детей
	...
	# Выбор стороны удаления (т.к. ситуации зеркальны)
	def choose_side(self, side, node, dad):
		...
		# Зеркально отражаем если правая сторона
		if side == 'right':
		...
		if sibling.color == 'BLACK':
			...
			self.to_big_rotate(sibling, dad)
			sibling.color = 'BLACK'
			dad.color = 'RED'
			self.del_node_without_child_black(node, node.parent)
	
	# Рекурсивный баланс если нет красных вершин в потомках
	...
	# 1a) Поворот когда брат черный правый внук красный
	def sib_black_rht_gr_son_red(self, sibling, dad, gr_son):
		self.to_big_rotate(sibling, dad)
		sibling.color = dad.color
		dad.color = gr_son.color = 'BLACK'
	
	# 1б) Поворот когда брат черный левый внук красный
	def sib_black_lft_gr_son_red(self, sibling, gr_son):
		self.to_big_rotate(gr_son, sibling)
		sibling.color, gr_son.color = 'RED', 'BLACK'
	
	# Удаление связей удаляемой ветки
